...
- **YAML** per tier: `QA_TestData.yml`, `PROD_TestData.yml`, … Gherkin `{key}` resolves via the parameter transformer. Keep new keys in the right tier file. Keys are test-data identifiers, not product copy. Do not invent course keys, product IDs, or ISBNs — **ask the operator** for the value that exists in that tier.
